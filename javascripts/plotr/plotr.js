...
Plotr.Base.getColorscheme = function(/*String*/color, /*String[]*/keys){
	...
	return Plotr.Base.generateColorscheme(Plotr.Base.colorSchemes.get(color) || color, keys);
};
...
*/
	getLegend: function(){
		...
		this.dataSets.keys().each(function(key){
			cStyle.backgroundColor = this.options.colorScheme.get(key);
...
*/
	_ieWaitForVML: function(/*String*/element, /*Object*/options){
		...
		if(!!(element)){
			this.renderStack.update({element: options});
		}
		...
		if(!!(this.renderDelay)){
			...
			this.renderDelay.stop();
			delete this.renderStack.get(element || this.canvasNode);
		}
...
*/
	render: function(/*String*/element, /*Object*/options) {
		...
		if(this.isIE){
			this.renderStack.each(function(pair) {
				if(typeof(pair.value) != 'function'){
					this.render(pair.key,pair.value);
					throw $break;
				}
			}.bind(this));
		}
...
*/
	_renderBarChart: function() {
		...
		var drawBar = function(bar, index) {
			var fillColor = new Plotr.Color(this.options.colorScheme.get(bar.name)).toRgbaString(parseFloat(this.options.fillOpacity));
			// Setup context.
	...
	_renderLineChart: function(){
		...
		var preparePath = function(storeName,index){
			...
			if(this.options.shouldFill){
	        	...
	        	cx.strokeStyle = this.options.colorScheme.get(storeName);
			    cx.stroke();
			}
		}.bind(this);
		...
		if(this.options.shouldFill){
			...
			var drawLine = function(storeName, index){
				...
				// Fill line.
				cx.fillStyle = new Plotr.Color(this.options.colorScheme.get(storeName)).toRgbaString(parseFloat(this.options.fillOpacity));
	            preparePath(storeName,index);
		        cx.fill();
	...
	_renderPieChart: function(){
		...
		this.slices.each(function(slice,i){
			...
			if(Math.abs(slice.startAngle - slice.endAngle) > 0.001){
				...
				cx.fillStyle = new Plotr.Color(this.options.colorScheme.get(slice.name)).toRgbaString(parseFloat(this.options.fillOpacity));;
